...
- **A ingestão de `compras` trabalha em lote, e a transação leva `timeout` explícito.** Uma fatura de 30 produtos rebentou em produção a 05/09/2026 com `P2028`: o antigo ciclo fazia três queries por linha (~92 no total) dentro de um `prisma.$transaction` sem opções, e o limite por omissão do Prisma são 5s — na VM aquilo levou 15,7s. Passou a ser uma query para procurar todos os nomes da fatura (`findMany` com `in` + `mode: 'insensitive'`) e três `createManyAndReturn` (produtos, variantes, preços), com `{ timeout: 20_000, maxWait: 10_000 }` como rede de segurança para a fatura de 500 produtos que o schema ainda permite. Medido contra Postgres real: 500 produtos passaram de 2498 ms para 446 ms. **Não voltar a pôr queries por linha de fatura dentro desta transação.** Duas consequências do lote a ter em conta: linhas repetidas na mesma fatura resolvem para um só produto (fica a primeira grafia vista), e quando há vários produtos com o mesmo nome escolhe-se o de id mais baixo que tenha exatamente uma variante — o `findFirst` antigo devolvia um qualquer e, se calhasse num produto já curado com 2+ variantes, criava mais um placeholder duplicado a cada importação.
- **`P2028` responde 503**, não o 500 genérico. O handler de erros em `app.ts` só traduzia `P2025`/`P2002`/`P2003`; tudo o resto caía num `{"error":"Erro interno do servidor"}` que não diz nada a quem lê os logs do n8n — foi preciso ir aos logs da VM (`pm2 logs carrinho-compras`) para descobrir onde rebentava. Nada é escrito quando a transação expira (há rollback), por isso repetir o pedido é seguro.
